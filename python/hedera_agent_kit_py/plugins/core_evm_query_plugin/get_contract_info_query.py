...
from hedera_agent_kit_py.shared.hedera_utils.mirrornode.types.evm import ContractInfo
...
def post_process(contract: ContractInfo) -> str:
    ...
    Also prints the raw response for debugging/visibility.
    """
    # Console log for visibility in user runs
    try:
        print("[get_contract_info_query] Mirror Node raw contract info:", contract)
    except Exception:
        # Best-effort logging; ignore any printing issues
        pass

    # Structured log for developers
    logging.error("[get_contract_info_query] Contract info: %s", contract)
...
async def get_contract_info_query(
    ...
    try:
        ...
        logging.info(ledger_id, parsed)

        info = await mirrornode_service.get_contract_info(parsed.contract_id)
