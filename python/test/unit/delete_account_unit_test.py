...
def mock_resolvers():
    ...
    with patch.object(
        AccountResolver, "is_hedera_address",
        side_effect=lambda addr: addr.count(".") == 2 and all(p.isdigit() for p in addr.split("."))
    ) as mock_is_addr, patch.object(
        AccountResolver, "get_default_account",
        return_value="0.0.1001"
    ) as mock_get_default:
...
def test_uses_provided_account_and_transfer_ids(mock_context, mock_client):
    ...
    params = DeleteAccountParameters(account_id="0.0.1234", transfer_account_id="0.0.4321")

    with patch.object(AccountResolver, "is_hedera_address", wraps=AccountResolver.is_hedera_address) as mock_is_addr:
        result = HederaParameterNormaliser.normalise_delete_account(params, mock_context, mock_client)
...
def test_defaults_transfer_account_id_when_missing(mock_context, mock_client):
    ...
    with patch.object(AccountResolver, "get_default_account", return_value="0.0.7777") as mock_get_default:
...
def test_raises_when_account_id_invalid(mock_context, mock_client):
    ...
    with patch.object(AccountResolver, "is_hedera_address", return_value=False):
        params = DeleteAccountParameters(account_id="not-hedera", transfer_account_id="0.0.1")

        with pytest.raises(ValueError, match="Account ID must be a Hedera address"):
            HederaParameterNormaliser.normalise_delete_account(params, mock_context, mock_client)
...
def test_raises_when_transfer_account_id_cannot_be_determined(mock_context, mock_client):
