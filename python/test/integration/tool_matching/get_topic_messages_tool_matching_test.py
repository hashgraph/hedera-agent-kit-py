"""Tool matching integration tests for get topic messages query tool.
...
@pytest.mark.asyncio
async def test_match_get_topic_messages_simple_request(
    agent_executor, toolkit, monkeypatch
):
    """Test that the tool matches a simple get topic messages request with explicit topic ID."""
    input_text = "Get messages for topic 0.0.1234"
    ...
    assert args[0] == GET_TOPIC_MESSAGES_QUERY_TOOL
    payload = args[1]
    assert payload.get("topic_id") == "0.0.1234"


@pytest.mark.asyncio
async def test_match_when_user_says_query_instead_of_get(
    agent_executor, toolkit, monkeypatch
):
    """Test that the tool matches when user says 'query' instead of 'get'."""
    input_text = "Query messages for topic 0.0.5555"
    config: RunnableConfig = {"configurable": {"thread_id": "1"}}
    ...
    assert args[0] == GET_TOPIC_MESSAGES_QUERY_TOOL
    payload = args[1]
    assert payload.get("topic_id") == "0.0.5555"
...
async def test_handle_various_natural_language_variations(
    ...
    variations = [
        ("Show messages for topic 0.0.2222", "0.0.2222"),
        ("Please get messages from topic 0.0.3333", "0.0.3333"),
        ("Display topic messages 0.0.4444", "0.0.4444"),
    ]
    config: RunnableConfig = {"configurable": {"thread_id": "1"}}
...
async def test_tool_available(toolkit):
    """Ensure the get topic messages tool is available in the toolkit."""
    tools = toolkit.get_tools()
    tool = next((t for t in tools if t.name == GET_TOPIC_MESSAGES_QUERY_TOOL), None)

    assert tool is not None
    assert tool.name == GET_TOPIC_MESSAGES_QUERY_TOOL
    assert (
