...
async def setup_accounts():
    ...
    executor_resp = await operator_wrapper.create_account(
        CreateAccountParametersNormalised(initial_balance=Hbar(5, in_tinybars=False), key=executor_key_pair.public_key())
    )
    ...
    recipient_resp = await executor_wrapper.create_account(
        CreateAccountParametersNormalised(initial_balance=0, key=operator_client.operator_private_key.public_key()))
    ...
    # Cleanup
    await return_hbars_and_delete_account(operator_wrapper, recipient_account_id, operator_client.operator_account_id)
    await return_hbars_and_delete_account(operator_wrapper, recipient_account_id2, operator_client.operator_account_id)
    await return_hbars_and_delete_account(executor_wrapper, executor_account_id, operator_client.operator_account_id)
...
async def test_single_hbar_transfer(setup_accounts):
    ...
    params = TransferHbarParameters(
        ...
        transfers=[TransferHbarEntry(account_id=str(recipient), amount=amount)] # passed in Hbars
    )
