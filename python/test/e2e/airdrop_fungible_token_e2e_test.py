...
async def setup_environment(operator_client, operator_wrapper):
    """Module-level environment setup using session-scoped operator fixtures."""
    # Executor account (Agent performing airdrops)
    executor_key = PrivateKey.generate_ed25519()
    executor_resp = await operator_wrapper.create_account(
        CreateAccountParametersNormalised(
            key=executor_key.public_key(),
            initial_balance=Hbar(UsdToHbarService.usd_to_hbar(3)),
        )
    )
    ...
    # Teardown - only cleanup executor resources, operator is managed by conftest.py
    lc_setup.cleanup()
